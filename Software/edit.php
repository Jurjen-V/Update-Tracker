...
$db = new PDO('mysql:host='.$dbhost.';dbname='.$dbname, $user, $pass);
$result_software = $db->prepare("SELECT * FROM usersoftware INNER JOIN software on usersoftware.Software_ID = Software.ID WHERE KoppelID = " . $_GET['KoppelID']);
$result_software->execute();
for($i=0; $row = $result_software->fetch(); $i++){
	$KoppelID = $row['KoppelID'];
...
if(isset($_POST['Save'])) {
    ...
    if ($error == 0) {
    $query = "UPDATE usersoftware SET Software_ID=:Software_ID WHERE KoppelID= :KoppelID";
    $stmt = $db->prepare($query);

    $stmt->bindValue(":KoppelID", $KoppelID, PDO::PARAM_STR);
	$stmt->bindValue(":Software_ID", $Software_ID, PDO::PARAM_STR);
	// $stmt->bindValue(":Version", $Versie, PDO::PARAM_STR);
